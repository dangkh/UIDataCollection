...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        uic.loadUi('UiFiles/preProcessMain.ui', self)
    ...
    def openFilePath(self):
        ...
        if path == "" or path is None:
            return
        subDir = str(path).split("/")
        parentDir = subDir[0]
        for x in subDir[1:-1]:
            parentDir = parentDir + '/' + x
        infoDir = parentDir + "/info.json"
        print(infoDir)
        try:
            with open(infoDir) as json_file:
                data = json.load(json_file)    
        except Exception as e:
            print("error read parent file")
            return
        ...
        infoSample = path + '/scenario.json'
        try:
            with open(infoSample) as json_file:
                data = json.load(json_file)    
        except Exception as e:
            print("error read parent file")
            return
        print(data)
        self.RecorderEdit.setText(data['RecorderEdit'])
        self.LocateEdit.setText(data['LocateEdit'])
        self.scenarioNumber.setValue(data['scenarioNumber'])
        self.sentenceEdit.setText(data['PlanDesc'])
        listEditatble = [self.NameEdit, self.AgeEdit, self.MaleEdit, self.FemaleEdit,
                         self.RecorderEdit, self.LocateEdit, self.scenarioNumber, self.sentenceEdit]
        for x in listEditatble:
            x.setEnabled(False)

        edfFile = path + '/EEG.edf'
        abc = mne.io.read_raw_edf(edfFile)
        abc.plot()

        cap = cv2.VideoCapture(path +"/FaceGesture.avi")
        ret, frame = cap.read()
        cap.release()
        ...
        w = self.CAM1.width()
        h = self.CAM1.height()
        self.CAM1.setPixmap(QPixmap.fromImage(image).scaled(w, h))

        img = cv2.imread('CAPTURE24.png')
        image = qimage2ndarray.array2qimage(img)
        self.EEG.setPixmap(QPixmap.fromImage(image))
        # fig = plt.figure(figsize=(5, 4), dpi=100)
        # self.canvas = FigureCanvasQTAgg(fig)
        # self.widEEG.addWidget(self.canvas)
        return
